...
export default class AbstractValidator {
  ...
  static isEmail(email) {
    if (!email) {
      throw new MessageError('IncorrectEmail', 'Отсутствует обязательный параметр email.', 400);
    }
  }

  static isPassword(password) {
    if (!password) {
      throw new MessageError('IncorrectPassword', 'Отсутствует обязательный параметр password.', 400);
    }
  }
